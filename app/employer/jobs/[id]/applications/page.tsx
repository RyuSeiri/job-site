...
const jobApplications = [
  {
    ...
    coverLetter: "貴社のフロントエンド開発に興味があり、これまでの経験を活かして貢献したいと考えています。",
    resumeUrl: "#",
  },
  {
    id: "APP-002",
    ...
    coverLetter: "フロントエンド開発の経験を活かし、ユーザー体験の向上に貢献したいです。",
    resumeUrl: "#",
  },
  {
    id: "APP-003",
  ...
  },
  {
    ...
    coverLetter: "モダンな技術スタックでの開発経験があり、即戦力として貢献できます。",
    resumeUrl: "#",
  },
  {
    id: "APP-005",
    ...
    coverLetter: "フロントエンド開発を学習中で、実務経験を積みたいと考えています。",
    resumeUrl: "#",
  },
]
...
export default function JobApplicationsPage({ params }: ApplicationsPageProps) {
  ...
  const filteredApplications = jobApplications.filter((app) => {
    const matchesTab =
      ...
      (activeTab === "reviewing" && ["reviewing", "interview"].includes(app.status)) ||
      (activeTab === "completed" && ["passed", "rejected"].includes(app.status))
  ...
  const statusCounts = {
    ...
    reviewing: jobApplications.filter((app) => ["reviewing", "interview"].includes(app.status)).length,
    completed: jobApplications.filter((app) => ["passed", "rejected"].includes(app.status)).length,
  }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
          <Link href="/employer" className="hover:text-foreground transition-colors">
            ダッシュボード
          </Link>
          <span>/</span>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-foreground mb-2">応募者管理</h1>
            <p className="text-muted-foreground">フロントエンドエンジニアの応募者一覧</p>
          </div>
          ...
          <CardContent>
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
              <TabsList className="grid w-full grid-cols-4">
                ...
                <TabsTrigger value="reviewing">選考中 ({statusCounts.reviewing})</TabsTrigger>
              ...
              <TabsContent value={activeTab} className="space-y-4">
                {filteredApplications.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-muted-foreground">該当する応募者がいません</p>
                  </div>
                ) : (
                        ...
                        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                          <div className="flex-1">
                            <div className="flex items-start justify-between mb-3">
                              <div>
                                <h3 className="text-xl font-semibold text-foreground">{application.candidateName}</h3>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                              <div className="text-sm">
                                <span className="text-muted-foreground">応募日:</span> {application.appliedDate}
                              </div>
                              <div className="text-sm">
                                <span className="text-muted-foreground">経験年数:</span> {application.experience}
                              </div>
                              <div className="text-sm">
                                <span className="text-muted-foreground">電話番号:</span> {application.phone}
                              </div>
                              <div className="text-sm">
                                <span className="text-muted-foreground">応募ID:</span> {application.id}
                              </div>
                            </div>

                            <div className="mb-4">
                              <p className="text-sm text-muted-foreground mb-2">スキル:</p>
                            ...
                            <div className="mb-4">
                              <p className="text-sm text-muted-foreground mb-2">志望動機:</p>
                          ...
                          <div className="flex flex-col gap-2 lg:ml-4 lg:w-48">
                            ...
                            {["new", "reviewing", "interview"].includes(application.status) && (
